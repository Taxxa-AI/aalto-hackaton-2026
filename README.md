...
The answer lives across the **avainhenkilölaki** (Finlex), a Verohallinto
bulletin, and a 2026 amendment that changed both the rate and the
validity window. Naïve RAG — embed everything, retrieve top-k, stuff
into a prompt — gets ~60% of the **easy** questions and falls over on
the hard ones because the right answer is *spread across documents
...
How you score yourself against this is up to you. Suggestions:

- **Fact recall** — does each key fact appear in your answer? (cheap, brittle)
- **LLM-as-judge** — prompt a strong model with the question, your answer,
  the reference, and the key facts. Ask for a 0/1 verdict + a one-sentence
  reason. (more honest, costs tokens)
- **Citation check** — does every numeric/statutory claim point at a source?

We'll evaluate submissions in a separate, sealed harness with our own
held-out questions, so don't over-fit to this exact bank.
...
4. **Agent loop.** Decompose ("rate → exceptions → effective date"),
   ...
   is low.
5. **Measure.** Score early, score often. A working bad pipeline
   beats a beautiful unfinished one.

If you can beat naïve RAG on the **hard** tier with something simpler —
do it. We'll be more impressed by a 70% score with three moving parts
than a 65% score with twelve.
...
1. **Correctness** — what % of questions does your system get right
   on our held-out set?
...
3. **Approach** — how well does your design model the domain?
   ...
   We want to see your reasoning, not just your score.

Show your work. A README in your submission explaining
"we tried X, it scored Y, here's why we ended up with Z" is worth
a lot more than clever code we have to reverse-engineer.
